...
Ferramenta web **standalone e independente** para coleta de dados biomecânicos: usa os sensores de orientação do celular (`DeviceOrientationEvent`) para monitorar, em tempo real, a inclinação do tronco de um atleta cadeirante durante atividade física.
...
2. Abra a página (precisa ser servida via **HTTPS** ou `localhost` — sensores de orientação não funcionam em origens inseguras).
...
Como o GitHub Pages serve tudo via HTTPS, o pedido de permissão de sensores (`DeviceOrientationEvent.requestPermission`, necessário no iOS) funciona normalmente a partir desse link.
